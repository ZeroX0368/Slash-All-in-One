...
// Autorole System Storage
const autoroles = new Map(); // guildId -> { roleId }

// Blacklist System Storage
const blacklistedServers = new Set(); // Set of blacklisted guild IDs
            ...
            .addSubcommand(subcommand =>
                subcommand
                    ...
                    .addStringOption(option =>
                        option
                            .setName('color')
                            .setDescription('The new color (hex code like #FF0000 or color name like red)')
                            .setRequired(true)
                    )
            ),
            ...
            .addSubcommand(subcommand =>
                subcommand
                    ...
                    .addStringOption(option =>
                        option
                            .setName('serverid')
                            .setDescription('The server ID to remove from blacklist')
                            .setRequired(true)
                    )
            )
            .addSubcommand(subcommand =>
            ...
            .addStringOption(option =>
                option
                    ...
                    .addChoices(
                        ...
                        { name: 'Autorole Commands', value: 'autorole' },
                        { name: 'Avatar Commands', value: 'avatar' },
                        { name: 'Blacklist Commands', value: 'blacklist' }
                    )
            )
    ];
...
client.on('interactionCreate', async interaction => {
    ...
    // Check if server is blacklisted (only allow blacklist command for owner)
    if (blacklistedServers.has(interaction.guild.id)) {
        // Only allow owner to use blacklist commands in blacklisted servers
        if (!(interaction.user.id === OWNER_ID && commandName === 'blacklist')) {
            const embed = new EmbedBuilder()
                .setTitle('🚫 Server Blacklisted')
                .setDescription('This server has been blacklisted from using bot commands.')
                .setColor('#ff0000')
                .setTimestamp();
    ...
    try {
        ...
        if (commandName === 'role') {
            ...
            if (subcommand === 'delete') {
                ...
                // Check bot permissions
                if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageRoles)) {
                    return await interaction.reply({ content: 'I need the "Manage Roles" permission to delete roles.', ephemeral: true });
                }
                ...
                // Check if the role is higher than the bot's highest role
                if (role.position >= interaction.guild.members.me.roles.highest.position) {
                    return await interaction.reply({ content: 'I cannot delete a role that is higher than or equal to my highest role.', ephemeral: true });
                }

                // Check if the role is higher than the user's highest role (unless they're the owner)
                if (interaction.guild.ownerId !== interaction.user.id && role.position >= interaction.member.roles.highest.position) {
                    return await interaction.reply({ content: 'You cannot delete a role that is higher than or equal to your highest role.', ephemeral: true });
                }

                // Check if role is managed (bot/integration role)
                if (role.managed) {
                    return await interaction.reply({ content: 'I cannot delete a managed role (bot/integration role).', ephemeral: true });
                }

                // Prevent deletion of @everyone role
                if (role.id === interaction.guild.id) {
                    return await interaction.reply({ content: 'I cannot delete the @everyone role.', ephemeral: true });
                }
                ...
                try {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .addFields(
                            ...
                            { name: 'Color', value: roleColor || 'No color', inline: true }
                        )
                        .setColor('#ff0000')
                        .setTimestamp();
                    ...
                    await interaction.reply({ content: 'Failed to delete role. Please check my permissions and try again.', ephemeral: true });
                }
            }
            ...
            if (subcommand === 'color') {
                ...
                // Check bot permissions
                if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageRoles)) {
                    return await interaction.reply({ content: 'I need the "Manage Roles" permission to change role colors.', ephemeral: true });
                }
                ...
                // Check if the role is higher than the bot's highest role
                if (role.position >= interaction.guild.members.me.roles.highest.position) {
                    return await interaction.reply({ content: 'I cannot modify a role that is higher than or equal to my highest role.', ephemeral: true });
                }

                // Check if the role is higher than the user's highest role (unless they're the owner)
                if (interaction.guild.ownerId !== interaction.user.id && role.position >= interaction.member.roles.highest.position) {
                    return await interaction.reply({ content: 'You cannot modify a role that is higher than or equal to your highest role.', ephemeral: true });
                }

                // Check if role is managed (bot/integration role)
                if (role.managed) {
                    return await interaction.reply({ content: 'I cannot modify a managed role (bot/integration role).', ephemeral: true });
                }

                // Prevent modification of @everyone role
                if (role.id === interaction.guild.id) {
                    return await interaction.reply({ content: 'I cannot modify the @everyone role.', ephemeral: true });
                }
                ...
                if (colorInput.toLowerCase() === 'none' || colorInput.toLowerCase() === 'default') {
                    colorValue = null; // Remove color
                } else if (colorInput.startsWith('#')) {
                    // Hex color
                    const hexPattern = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/;
                    if (!hexPattern.test(colorInput)) {
                        return await interaction.reply({ content: 'Invalid hex color format. Please use format like #FF0000 or #F00.', ephemeral: true });
                    }
                    colorValue = colorInput.toUpperCase();
                } else if (colorNames[colorInput.toLowerCase()]) {
                    // Color name
                    colorValue = colorNames[colorInput.toLowerCase()];
                } else {
                    return await interaction.reply({ content: 'Invalid color. Please use a hex code (like #FF0000) or a color name (like red, blue, green, etc.), or "none" to remove color.', ephemeral: true });
                }

                const oldColor = role.hexColor || 'No color';

                try {
                    await role.setColor(colorValue, `Role color changed by ${interaction.user.tag}`);

                    const newColor = colorValue || 'No color';

                    const embed = new EmbedBuilder()
                        .setTitle('Role Color Changed')
                        .setDescription(`Successfully changed color for role ${role}`)
                        .addFields(
                            { name: 'Role', value: `${role}`, inline: true },
                            { name: 'Old Color', value: oldColor, inline: true },
                            { name: 'New Color', value: newColor, inline: true }
                        )
                        .setColor(colorValue || '#0099ff')
                        .setTimestamp();

                    await interaction.reply({ embeds: [embed] });
                } catch (error) {
                    console.error('Error changing role color:', error);
                    await interaction.reply({ content: 'Failed to change role color. Please check my permissions and ensure the color is valid.', ephemeral: true });
                }
            }
        }
        ...
        if (commandName === 'autorole') {
            ...
            if (subcommand === 'add') {
                ...
                // Check bot permissions
                if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageRoles)) {
                    return await interaction.reply({ content: 'I need the "Manage Roles" permission to assign autoroles.', ephemeral: true });
                }

                const role = options.getRole('role');
                const roleId = options.getString('role_id');

                // Check if either role or role_id is provided
                if (!role && !roleId) {
                    return await interaction.reply({ content: 'Please provide either a role or a role ID.', ephemeral: true });
                }

                let targetRole = role;

                // If role_id is provided, try to fetch the role
                if (roleId && !role) {
                    try {
                        targetRole = await interaction.guild.roles.fetch(roleId);
                    } catch (error) {
                        return await interaction.reply({ content: 'Invalid role ID provided.', ephemeral: true });
                    }
                }
                ...
                // Check if the role is higher than the user's highest role (unless they're the owner)
                if (interaction.guild.ownerId !== interaction.user.id && targetRole.position >= interaction.member.roles.highest.position) {
                    return await interaction.reply({ content: 'You cannot set an autorole that is higher than or equal to your highest role.', ephemeral: true });
                }

                // Check if role is managed (bot/integration role)
                if (targetRole.managed) {
                    return await interaction.reply({ content: 'Cannot set a managed role (bot/integration role) as autorole.', ephemeral: true });
                }

                // Store the autorole configuration
                autoroles.set(interaction.guild.id, { roleId: targetRole.id });

                const embed = new EmbedBuilder()
                    .setTitle('Autorole Set')
                    .setDescription(`Successfully set ${targetRole} as the autorole. New members will automatically receive this role when they join.`)
                    .addFields(
            ...
            if (subcommand === 'remove') {
                ...
                if (!autoroles.has(interaction.guild.id)) {
                    return await interaction.reply({ content: 'No autorole is currently configured for this server.', ephemeral: true });
                }
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription('Successfully removed the autorole configuration. New members will no longer automatically receive a role.')
                    .setColor('#ff6b6b')
                    .setTimestamp();
            ...
            if (subcommand === 'list') {
                ...
                const autoroleData = autoroles.get(interaction.guild.id);
                const role = interaction.guild.roles.cache.get(autoroleData.roleId);

                if (!role) {
                    // Role was deleted, remove from storage
                    autoroles.delete(interaction.guild.id);
                    return await interaction.reply({ content: 'The configured autorole no longer exists. Configuration has been removed.', ephemeral: true });
                }

                // Get members with this role (limit to first 10 for display)
                const membersWithRole = role.members.map(member => member.user.tag).slice(0, 10);
                const membersList = membersWithRole.length > 0 
                    ? membersWithRole.join('\n') + (role.members.size > 10 ? `\n... and ${role.members.size - 10} more` : '')
                    : 'No members have this role';

                const embed = new EmbedBuilder()
                    .setTitle('Autorole Configuration')
                    .setDescription('Current autorole settings for this server')
                    .addFields(
                        { name: 'Role', value: `${role}`, inline: true },
                        { name: 'Role Name', value: role.name, inline: true },
                        { name: 'Role ID', value: role.id, inline: true },
                        { name: 'Total Members with Role', value: role.members.size.toString(), inline: true },
                        { name: 'Members with Role', value: membersList, inline: false }
                    )
                    .setColor('#0099ff')
                    .setTimestamp();
        ...
        if (commandName === 'blacklist') {
            ...
            if (interaction.user.id !== OWNER_ID) {
                return await interaction.reply({ content: 'This command is only available to the bot owner.', ephemeral: true });
            }
            ...
            if (subcommand === 'server') {
                ...
                if (!/^\d{17,19}$/.test(serverId)) {
                    return await interaction.reply({ content: 'Please provide a valid server ID (17-19 digits).', ephemeral: true });
                }

                // Check if server is already blacklisted
                if (blacklistedServers.has(serverId)) {
                    return await interaction.reply({ content: 'This server is already blacklisted.', ephemeral: true });
                }

                // Add to blacklist
                blacklistedServers.add(serverId);

                // Try to get server name for better display
                ...
                try {
                    ...
                    console.log('Could not fetch guild name:', error);
                }

                const embed = new EmbedBuilder()
                    .setTitle('🚫 Server Blacklisted')
                    .setDescription(`Successfully blacklisted server: **${serverName}**`)
                    .addFields(
                        { name: 'Server ID', value: serverId, inline: true },
                        { name: 'Total Blacklisted', value: blacklistedServers.size.toString(), inline: true }
                    )
                    .setColor('#ff0000')
                    .setTimestamp();
            ...
            if (subcommand === 'remove') {
                ...
                if (!blacklistedServers.has(serverId)) {
                    return await interaction.reply({ content: 'This server is not blacklisted.', ephemeral: true });
                }
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`Successfully removed server from blacklist: **${serverName}**`)
                    .addFields(
                        { name: 'Server ID', value: serverId, inline: true },
                        { name: 'Total Blacklisted', value: blacklistedServers.size.toString(), inline: true }
                    )
            ...
            if (subcommand === 'list') {
                if (blacklistedServers.size === 0) {
                    return await interaction.reply({ content: 'No servers are currently blacklisted.', ephemeral: true });
                }

                const blacklistedArray = Array.from(blacklistedServers);
                const itemsPerPage = 10;
                const totalPages = Math.ceil(blacklistedArray.length / itemsPerPage);
                let currentPage = 0;

                const generateEmbed = (page) => {
                    const start = page * itemsPerPage;
                    const end = start + itemsPerPage;
                    const serverList = blacklistedArray.slice(start, end).map((serverId, index) => {
                        const globalIndex = start + index + 1;
                        let serverName = 'Unknown Server';
                        ...
                        try {
                            ...
                            // Server name will remain 'Unknown Server'
                        }

                        return `\`${globalIndex}.\` **${serverName}** (${serverId})`;
                    }).join('\n');

                    return new EmbedBuilder()
                        .setTitle(`🚫 Blacklisted Servers - Page ${page + 1}/${totalPages}`)
                ...
                if (totalPages > 1) {
                    ...
                    collector.on('collect', async i => {
                        ...
                        if (i.customId === 'blacklist_next') {
                            currentPage++;
                        } else if (i.customId === 'blacklist_prev') {
                            currentPage--;
                        }
            ...
            if (!category) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: '⚙️ Autorole Commands', value: '`/help autorole` - Automatic role add', inline: true },
                ...
                switch (category) {
                    ...
                    case 'role':
                        embed = new EmbedBuilder()
                            ...
                            .addFields(
                                ...
                                { name: '/role delete <role>', value: 'Delete a role permanently. Both you and the bot need "Manage Roles" permission. The role must be lower than both your highest role and the bot\'s highest role.', inline: false },
                                { name: '/role color <role> <color>', value: 'Change the color of a role. You can use hex codes (#FF0000), color names (red, blue, etc.), or "none" to remove color. Both you and the bot need "Manage Roles" permission.', inline: false }
                            )
                            .setColor('#e74c3c');
                        break;
                    ...
                    case 'autorole':
                        embed = new EmbedBuilder()
                            .setTitle('⚙️ Autorole Commands')
                            .setDescription('Commands for automatic role assignment (requires Manage Roles permission)')
                            .addFields(
                                { name: '/autorole add <role/role_id>', value: 'Set up a role to be automatically assigned to new members when they join the server. You can specify either a role or role ID.', inline: false },
                                { name: '/autorole remove', value: 'Remove the current autorole configuration. New members will no longer automatically receive a role.', inline: false },
                                { name: '/autorole list', value: 'Display the current autorole configuration for this server, including role details and member count.', inline: false }
                            )
                            .setColor('#ffa500');
                        break;
                    ...
                    case 'blacklist':
                        embed = new EmbedBuilder()
                            .setTitle('🚫 Blacklist Commands (Owner Only)')
                            .setDescription('Commands for server blacklist management')
                            .addFields(
                                { name: '/blacklist server <serverid>', value: 'Blacklist a server from using bot commands. When a server is blacklisted, members cannot use any bot commands.', inline: false },
                                { name: '/blacklist remove <serverid>', value: 'Remove a server from the blacklist, allowing them to use bot commands again.', inline: false },
                                { name: '/blacklist list', value: 'Display all currently blacklisted servers with pagination.', inline: false }
                            )
                            .setColor('#ff0000');
                        break;
...
// Autorole system - Handle new member joins
client.on('guildMemberAdd', async member => {
    const guildId = member.guild.id;
    
    if (autoroles.has(guildId)) {
        const autoroleData = autoroles.get(guildId);
        const role = member.guild.roles.cache.get(autoroleData.roleId);
        
        if (role) {
            try {
                await member.roles.add(role, 'Autorole Add to new member');
                console.log(`Add autorole ${role.name} to ${member.user.tag} in ${member.guild.name}`);
            } catch (error) {
            ...
            console.log(`Autorole configuration removed for ${member.guild.name} - role no longer exists`);
        }
    }
});
